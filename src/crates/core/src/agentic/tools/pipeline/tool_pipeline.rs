...
impl ToolPipeline {
    ...
    async fn execute_parallel(&self, task_ids: Vec<String>) -> BitFunResult<Vec<ToolExecutionResult>> {
    ...
    async fn execute_sequential(&self, task_ids: Vec<String>) -> BitFunResult<Vec<ToolExecutionResult>> {
    ...
    pub async fn confirm_tool(&self, tool_id: &str, updated_input: Option<serde_json::Value>) -> BitFunResult<()> {
        let task = self.state_manager
            .get_task(tool_id)
            .ok_or_else(|| BitFunError::NotFound(format!("Tool task not found: {}", tool_id)))?;
